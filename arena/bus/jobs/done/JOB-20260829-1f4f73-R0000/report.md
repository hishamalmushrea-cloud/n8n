# تقرير فحص أمني — retest: DEMO — فحص معمل الاختبار (10 findings)

- **المعرّف:** `JOB-20260829-1f4f73-R0000`
- **الهدف:** `http://localhost:8090` (url)
- **الوضع:** retest — **الأولوية:** P2
- **التاريخ:** 2026-08-29T20:01:12.611Z
- **المنفّذ:** Arena Agent Mode (بدون API خارجي)
- **الخلاصة:** 11 نتيجة — 0 حاسمة/عالية

## الجدول

| | الخطورة | CVSS | العنوان | مُثبَت | الحالة |
|---|---|---|---|---|---|
| F-999df2 | critical | 9.1 | نقطة نهاية تصحيحية مكشوفة تُرجع كل كلمات المرور بوضوح | ✅ | fixed-verified |
| F-9bbe6d | high | 8.1 | BOLA/IDOR في /api/orders/:id — أي مستخدم يقرأ طلبات الآخرين مع أرقام البطاقات | ✅ | fixed-verified |
| F-ff4116 | high | 8.8 | Mass assignment في POST /api/users/me يسمح بترقية الدور إلى admin | ✅ | fixed-verified |
| F-4a6f8c | high | 8.6 | SSRF: الخادم يجلب أي URL من مدخل المستخدم (منافذ داخلية + رابط metadata) | ✅ | fixed-verified |
| F-a52038 | medium | 6.1 | XSA من نوع Reflected عبر /search?q= بدون ترميز HTML | ✅ | fixed-verified |
| F-218087 | medium | 5.3 | عدم وجود حد لمحاولات الدخول + رسائل تمييز وجود المستخدم | ✅ | fixed-verified |
| F-705ab6 | medium | 6.5 | كلمات المرور تُخزّن وتُعاد بنص صريح في استجابات الـ API | ✅ | fixed-verified |
| F-1233a4 | low | 3.7 | SSRF/internal error: تسريب Stack Trace ومسار الملفات من المعالج العام | ✅ | fixed-verified |
| F-65472a | low | 3.7 | ترويسات أمنية ناقصة + كشف البنية (server / x-powered-by) وغياب HSTS/CSP/Frameguard | ✅ | fixed-verified |
| F-016e36 | medium | 5.9 | JWT: الخوارزمية غير مثبّتة ولا تحقق من iss/aud، وسر التوقيع في الكود | — | fixed-verified |
| F-ab5367 | critical | 9.8 | إثبات إضافي: تزوير JWT بـ alg=none كان يعمل فعلاً على النسخة غير المُؤمَّنة | ✅ | fixed-verified |
...
## CRITICAL — إثبات إضافي: تزوير JWT بـ alg=none كان يعمل فعلاً على النسخة غير المُؤمَّنة (`F-ab5367`)

أثناء إعادة الفحص شُغّلت النسخة الأصلية على :8091 وقُبل توكن موقَّع بـ alg=none (توقيع فارغ/قديم) وأعاد بيانات طلبات مستخدمين — أي أن نتيجة F-016e36 التي سُجّلت سابقاً بـ verified:false يجب أن تُرفع إلى critical/verified.

**خطوات الإثبات (PoC):**
- خذ توكن صالح وقسّمه إلى h.p.s
- استبدل الرأس بـ base64url({"alg":"none","typ":"JWT"}) وأبقِ p.s كما هما
- GET /api/orders/1001 → 200 بدل 401

**الإصلاح المقترح:** ثبّت الخوارزمية المصرّح بها واحرص على 3 أقسام ورفض أي رأس آخر (مطبَّق في server.secured.mjs: head.alg !== 'HS256' → null)
